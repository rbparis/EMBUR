...
export type AppPage = "Today" | "Customers" | "Conversations" | "Operations" | "Business" | "Settings";
...
export default function AppSidebar({ activePage, onPageChange }: AppSidebarProps) {
  return (
    <aside className="flex min-w-0 flex-col bg-slate-950 p-5 text-white md:p-6">
      <EmburLogo light />
      <nav aria-label="EMBUR application" className="mt-8 flex gap-2 overflow-x-auto pb-2 lg:mt-10 lg:block lg:space-y-2 lg:overflow-visible lg:pb-0">
        {navigation.map((item) => {
          const isActive = activePage === item.page;
          return (
            <button type="button" key={item.page} onClick={() => onPageChange(item.page)} aria-current={isActive ? "page" : undefined}
              className={`flex shrink-0 items-center gap-3 rounded-xl px-4 py-3 text-left font-medium transition lg:w-full ${isActive ? "bg-white text-slate-950 shadow-sm" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}>
      ...
      <div className="mt-auto hidden rounded-2xl border border-white/10 bg-white/5 p-4 lg:block">
        <p className="text-xs font-bold uppercase tracking-wider text-orange-300">Atlas is watching</p>
        <p className="mt-2 text-lg font-bold">Your next move, prioritized.</p>
        <p className="mt-1 text-sm leading-relaxed text-slate-400">Open Today whenever the day feels noisy.</p>
      </div>
    </aside>
  );
}
